...
// Stores reviews in component state. Uses lucide-react icons.
  ...
  return (
    <div className="fb-stars" onMouseLeave={onLeave}>
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          <Star size={32} fill={star <= (hovered || rating) ? "#FBBF24" : "none"} color={star <= (hovered || rating) ? "#FBBF24" : "#D1D5DB"} />
        </button>
      ))}
      {(hovered || rating) > 0 && (
    ...
    <div className="fb-page">
      ...
      <div className="fb-container">
            ...
            <div className="fb-summary-stars">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star
                  ...
                  color={s <= Math.round(Number(avgRating)) ? "#FBBF24" : "#D1D5DB"}
                />
              ))}
            </div>
        ...
        <div className="fb-card">
          ...
          {success && (
            <div className="fb-success" style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <CheckCircle size={15} />
              Thank you! Your review has been submitted.
            </div>
          )}
          {error && (
            <div className="fb-error" style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <AlertCircle size={15} />
              {error}
            </div>
          )}
        ...
        {reviews.length > 0 && (
          <div className="fb-reviews">
            <h2 className="fb-reviews-title">
              ...
              All Reviews
              <span className="fb-reviews-badge">{reviews.length}</span>
            </h2>
